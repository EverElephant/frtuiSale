    ...
    $.fn.extend({
        openMask:function(){
            ...
            if(!loadDiv || !loadDiv[0]){    // add Mask
                ...
                var loadDiv=$("<div class='_mask_loadDiv' style='display: block;width: 100%;height: 100%;background:#ccc;opacity:0.5;-ms-filter: progid:DXImageTransform.Microsoft.Alpha(opacity=60);filter: progid:DXImageTransform.Microsoft.Alpha(opacity=60);position: absolute;top: 0;left: 0;z-index: 2000; width:" + w_w + "px;height:" + w_h + "px;'></div>");
                var contentDiv=$("<div class=\"_mask_content\" style=\"_width:160px; height:22px; position: fixed; position:absolute; top: 50%;left: 50%;margin-left: -74px;margin-top: -14px;border: solid 2px #8DB2E3;background: #fff url('../images/1.gif') no-repeat 10px 10px;display: block;padding: 9px 5px 2px 35px;text-align: left;z-index: 2001;\" ><font color='#ff4500'>数据玩命加载中...</font></div>");
...
document.write("<div class=\"_mask_loadDiv\">");
document.write("    <div class=\"_mask_content\">");
document.write("       </div>");
document.write("   </div>");
document.write("<div id=\"mb_target\">");
document.write("</div>");
//============这部分是加数据加载的蒙板的==================

function qiupuaddr() {
    var tempStr = "http://erpapi.lechun.cc/";
    return tempStr
};
...
function hostaddr() {
    var tempStr = "http://localhost";
    return tempStr
};


function showMask() {
    $("#backgroundChenguo").css("z-index", "9999");
    $("#progressBarChenguo").css("z-index", "9999");
    $("#backgroundChenguo").show();
    $("#progressBarChenguo").show()
};

function hideMask() {
    $("#backgroundChenguo").css("z-index", "-9999");
    $("#progressBarChenguo").css("z-index", "-9999");
    $("#backgroundChenguo").hide();
    $("#progressBarChenguo").hide()
};
...
function showTip() {
    ...
    $("#backgroundChenguo").css("width", w_w);
    if (w_h > d_h) {
        $("#backgroundChenguo").css("height", w_h)
    } else {
        $("#backgroundChenguo").css("height", d_h)
    };
    $("#progressBarChenguo").text("提交数据中,请稍候...");
    $("#progressBarChenguo").center();
    $("#backgroundChenguo").css("opacity", "0.5");
    $("#backgroundChenguo,#progressBarChenguo").fadeIn()
};

function hideTip() {
    $("#backgroundChenguo,#progressBarChenguo").fadeOut();
    $("#progressBarChenguo").hide();
    $("#backgroundChenguo").hide()
};
...
function invokeApi(cmd, jdata, call) {
    ...
    $.each(jdata, function(i) {
        var key1 = i;
        if (key1.toString() != "fresh" && key1.toString() != "call_id" && key1.toString() != "_" && key1.toString() != "callback") {
            p_str = p_str + key1.toString() + ","
        }
    });
    if (p_str.length > 0) {
    ...
    for (var j = 0; j < b.length; j++) {
        $.each(jdata, function(m) {
            var key1 = m;
            if (key1.toString() == b[j].toString()) {
                f_str = f_str + b[j].toString() + "=" + jdata[m] + "|"
            }
        })
    };
    if (f_str.length > 0) {
    ...
    console.log(f_str)
    $.getJSON(qiupuaddr() + cmd + "?ticket=" + ticket + "&sign=" + b64_md5(f_str) + "&callback=?",
        jdata,
        function(ret){
            $("#mb_target").closeMask();
            if($.isFunction(call)){
                call(ret);
            }else if($.isFunction(jdata)){
                jdata(ret);
...
/*
    $.ajax({
        url:qiupuaddr() + cmd + "?ticket=" + ticket + "&sign=" + b64_md5(f_str) + "&callback=?",
        data:jdata,
        type:"GET",
        dataType:"json",
...
function clearCookie() {
    ...
    $.cookie("user_id", null, {path:"/"},"gekec.com");
    $.cookie("user_name", null, {path:"/"},"gekec.com");
    $.cookie("display_name", null, {path:"/"},"gekec.com");
    $.cookie("ticket", null, {path:"/"},"gekec.com");
    $.cookie("user_type", null, {path:"/"},"gekec.com");
    $.cookie("auth_type", null, {path:"/"},"gekec.com");
    $.cookie("img_l", null, {path:"/"},"gekec.com");
    $.cookie("img_s", null, {path:"/"},"gekec.com");
    $.cookie("USER_MOD", null, {path:"/"},"gekec.com");
    $.cookie("USER_CHANNEL", null, {path:"/"},"gekec.com");
    $.cookie("user_id", null, {path:"/"},"www.gekec.com");
    $.cookie("user_name", null, {path:"/"},"www.gekec.com");
    $.cookie("display_name", null, {path:"/"},"www.gekec.com");
    $.cookie("ticket", null, {path:"/"},"www.gekec.com");
    $.cookie("user_type", null, {path:"/"},"www.gekec.com");
    $.cookie("auth_type", null, {path:"/"},"www.gekec.com");
    $.cookie("img_l", null, {path:"/"},"www.gekec.com");
    $.cookie("img_s", null, {path:"/"},"www.gekec.com");
    $.cookie("USER_MOD", null, {path:"/"},"www.gekec.com");
    $.cookie("USER_CHANNEL", null, {path:"/"},"www.gekec.com");
};
